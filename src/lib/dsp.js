...
//   vascular axes    SDPTG (2nd derivative) wave ratios, max-slope aligned
//
// Deliberately NOT included: any demographic lookup table. Earlier versions
// derived most of the dosha result from hand-authored anchor tables keyed on
// age/gender/weight/height; those numbers were invented and have been removed
// rather than retuned.
// ===========================================================================
...
function sMean(a) { return a.length ? a.reduce((x, y) => x + y, 0) / a.length : NaN; }
function sMedian(a) {
  if (!a.length) return NaN;
  const s = [...a].sort((x, y) => x - y);
  const m = s.length >> 1;
...
// Savitzky-Golay-style smoothing via repeated moving average. Essential before
// differentiating twice: the second derivative amplifies high-frequency noise
// quadratically, so an unsmoothed SDPTG is unusable.
function sSmooth(arr, w, passes = 2) {
  let cur = arr.slice();
...
function sDeriv(a) {
  ...
  for (let i = 1; i < a.length - 1; i++) d[i] = (a[i + 1] - a[i - 1]) / 2;
  if (a.length > 2) { d[0] = d[1]; d[a.length - 1] = d[a.length - 2]; }
  return d;
}

function sResample(seg, n) {
  const out = new Array(n);
  const last = seg.length - 1;
  for (let i = 0; i < n; i++) {
    const pos = (i / (n - 1)) * last;
    const lo = Math.floor(pos), hi = Math.min(last, lo + 1);
    const f = pos - lo;
    out[i] = seg[lo] * (1 - f) + seg[hi] * f;
  }
  return out;
}

// Find the index of the largest local maximum within [lo, hi).
function argMaxLocal(arr, lo, hi) {
  lo = Math.max(1, lo); hi = Math.min(arr.length - 1, hi);
  let best = -1;
  for (let i = lo; i < hi; i++) {
  ...
  if (best === -1) { // no local max: fall back to plain maximum
    for (let i = lo; i < hi; i++) if (best === -1 || arr[i] > arr[best]) best = i;
  }
  return best;
}

function argMinLocal(arr, lo, hi) {
  lo = Math.max(1, lo); hi = Math.min(arr.length - 1, hi);
  let best = -1;
  for (let i = lo; i < hi; i++) {
    if (arr[i] <= arr[i - 1] && arr[i] <= arr[i + 1]) {
...
const TEMPLATE_N = 200;      // template resolution
const PRE_FRACTION = 0.20;   // how much of the window sits before the upstroke

/**
 * Build an ensemble pulse template aligned on maximum upslope.
 */
function buildAlignedTemplate(buffer, beats, sampleIntervalMs) {
  if (beats.length < 5) return null;

  // Typical cycle length in samples, from the beat series.
  const gaps = [];
  for (let i = 1; i < beats.length; i++) gaps.push(beats[i] - beats[i - 1]);
  const cyc = sMedian(gaps);
  if (!(cyc >= 6)) return null;

  // Lightly smooth the whole signal before differentiating, so the upstroke
  // landmark is stable against sample-level noise.
  const smoothW = Math.max(3, Math.round(cyc * 0.06) | 1);
  const sig = sSmooth(buffer, smoothW, 1);
  const d1 = sDeriv(sig);

  // For each beat, locate max(dV/dt) on the upstroke leading into the peak.
  const anchors = [];
  for (const b of beats) {
    const lo = Math.max(1, Math.round(b - cyc * 0.45));
    const hi = Math.min(sig.length - 1, b + 1);
    if (hi - lo < 3) continue;
    let best = lo;
    for (let i = lo; i < hi; i++) if (d1[i] > d1[best]) best = i;
    anchors.push(best);
  }
  if (anchors.length < 4) return null;

  const pre = Math.round(cyc * PRE_FRACTION);
  const post = Math.round(cyc * (1 - PRE_FRACTION));

  const cycles = [];
  let rejected = 0;
  for (const u of anchors) {
    const s = u - pre, e = u + post;
    if (s < 0 || e >= buffer.length) { rejected++; continue; }
    const seg = buffer.slice(s, e + 1);
    let mn = seg[0], mx = seg[0];
    for (const v of seg) { if (v < mn) mn = v; if (v > mx) mx = v; }
    const rng = mx - mn;
    if (!(rng > 0)) { rejected++; continue; }
    cycles.push(sResample(seg.map(v => (v - mn) / rng), TEMPLATE_N));
  }
  if (cycles.length < 4) return null;

  const template = new Array(TEMPLATE_N);
  for (let i = 0; i < TEMPLATE_N; i++) template[i] = sMedian(cycles.map(c => c[i]));

  // Alignment quality: mean per-point spread across cycles. Low spread means
  // the beats genuinely snapped into phase.
  const spread = [];
  for (let i = 0; i < TEMPLATE_N; i++) {
    const col = cycles.map(c => c[i]);
    const m = sMean(col);
    spread.push(Math.sqrt(sMean(col.map(v => (v - m) * (v - m)))));
  }
  const meanSpread = sMean(spread);

  return {
    template,
    cyclesUsed: cycles.length,
    cyclesRejected: rejected,
    cycleSamples: cyc,
    cycleMs: cyc * sampleIntervalMs,
    alignmentQuality: +Math.max(0, 1 - meanSpread * 4).toFixed(4),
    anchorFrac: PRE_FRACTION,
  };
}

/**
 * Locate the SDPTG a,b,c,d,e waves and derive the three axes.
 */
function analyseSdptg(buffer, beats, sampleIntervalMs) {
  const built = buildAlignedTemplate(buffer, beats, sampleIntervalMs);
  if (!built) {
    return { ok: false, reason: 'could not build an aligned pulse template (need >=5 clean beats)' };
  }

  const N = TEMPLATE_N;
  // Smooth the template before double differentiation.
  const tpl = sSmooth(built.template, 9, 2);
  const d1 = sSmooth(sDeriv(tpl), 7, 1);
  const d2 = sSmooth(sDeriv(d1), 7, 1);   // SDPTG / APG

  const frac = i => i / (N - 1);
  const toMs = i => (i / (N - 1)) * built.cycleMs;

  // Systolic peak P1 on the raw template.
  const upstroke = Math.round(PRE_FRACTION * (N - 1));
  let p1 = upstroke;
  const p1Hi = Math.min(N - 2, Math.round(N * 0.55));
  for (let i = upstroke; i < p1Hi; i++) if (tpl[i] > tpl[p1]) p1 = i;

  // ---- SDPTG wave search over PHYSIOLOGICAL windows ----------------------
  // Chaining each search off the previous wave's position (d searched just
  // after c, e just after d) fails badly: every bump in the pulse produces its
  // own +/-/+ triplet in the second derivative, so a chained search finds
  // c, d and e all inside the TIDAL wave's own triplet and never reaches the
  // dicrotic wave at all. Verified: with that approach e/a tracked the tidal
  // amplitude exactly (0.009 -> 0.801 as a2 went 0.0 -> 0.8) while being
  // almost blind to the actual dicrotic amplitude.
  //
  // Fix: search each wave inside its own physiologically expected window,
  // expressed as a fraction of the template (which spans exactly one cardiac
  // cycle, so these fractions are heart-rate invariant), while still enforcing
  // strict ordering a < b < c < d < e.
  const win = (loFrac, hiFrac, after) => {
    let lo = Math.max(1, Math.round(N * loFrac));
    if (after !== undefined) lo = Math.max(lo, after + 2);
    const hi = Math.min(N - 2, Math.round(N * hiFrac));
    return [lo, Math.max(lo + 2, hi)];
  };

  // a: peak ejection acceleration, straddling the upstroke landmark.
  let [aLo, aHi] = win(0.06, 0.30);
  aHi = Math.min(aHi, Math.max(aLo + 2, p1));
  const a = argMaxLocal(d2, aLo, aHi);

  // b: systolic deceleration, shortly after a.
  const [bLo, bHi] = win(0.12, 0.45, a);
  const b = argMinLocal(d2, bLo, bHi);

  // c: reflected / tidal wave.
  const [cLo, cHi] = win(0.22, 0.55, b);
  const cIdx = argMaxLocal(d2, cLo, cHi);

  // d: dicrotic notch — mid-to-late systole into early diastole.
  const [dLo, dHi] = win(0.34, 0.72, cIdx);
  const dIdx = argMinLocal(d2, dLo, dHi);

  // e: dicrotic rebound, in diastole.
  const [eLo, eHi] = win(0.44, 0.92, dIdx);
  const eIdx = argMaxLocal(d2, eLo, eHi);

  const A = d2[a];
  if (!(A > 0)) {
    return { ok: false, reason: 'SDPTG a-wave not positive — signal too noisy or distorted' };
  }

  // Standard SDPTG ratios, all normalised by the a-wave so they are
  // dimensionless and independent of signal gain.
  const b_a = d2[b] / A;
  const c_a = d2[cIdx] / A;
  const d_a = d2[dIdx] / A;
  const e_a = d2[eIdx] / A;
  // Takazawa aging index — a recognised composite of the same waves.
  const agingIndex = (d2[b] - d2[cIdx] - d2[dIdx] - d2[eIdx]) / A;

  // ---- three independent axes -------------------------------------------
  // 1) EJECTION SHARPNESS. Peak acceleration relative to how long the
  //    upstroke takes: a fast, forceful ejection has large a and short crest
  //    time. Normalised by cycle so it is rate-independent.
  const crestTimeFrac = (p1 - upstroke) / (N - 1);
  const crestNorm = Math.max(1e-4, crestTimeFrac);
  // A is in (0..1 template units) per sample^2, so scale by N^2 to express it
  // per unit-cycle-fraction^2 — that makes it independent of TEMPLATE_N.
  const aPerCycle2 = A * (N - 1) * (N - 1);
  const sharpnessRaw = aPerCycle2 / crestNorm;
  // 2) ARTERIAL COMPLIANCE. The c and d waves carry the reflected wave.
  //    Clinically, less negative d/a and higher c/a indicate greater
  //    augmentation from reflection (stiffer vessels, stronger tidal wave).
  const complianceRaw = Math.max(0, c_a - d_a);
  // 3) DICROTIC REBOUND. The e wave.
  const reboundRaw = Math.max(0, e_a);

  return {
    ok: true,
    axes: {
      ejection_sharpness: +sharpnessRaw.toFixed(5),
      arterial_compliance: +complianceRaw.toFixed(5),
      dicrotic_rebound: +reboundRaw.toFixed(5),
    },
    sdptg: {
      a_index: a, a_frac: +frac(a).toFixed(4), a_ms: +toMs(a).toFixed(1),
      b_index: b, b_frac: +frac(b).toFixed(4), b_a: +b_a.toFixed(4),
      c_index: cIdx, c_frac: +frac(cIdx).toFixed(4), c_a: +c_a.toFixed(4),
      d_index: dIdx, d_frac: +frac(dIdx).toFixed(4), d_a: +d_a.toFixed(4),
      e_index: eIdx, e_frac: +frac(eIdx).toFixed(4), e_a: +e_a.toFixed(4),
      aging_index: +agingIndex.toFixed(4),
    },
    morphology: {
      p1_index: p1,
      crest_time_frac: +crestTimeFrac.toFixed(4),
      crest_time_ms: +(crestTimeFrac * built.cycleMs).toFixed(1),
      cycle_ms: +built.cycleMs.toFixed(1),
    },
    template: tpl,
    sdptgCurve: d2,
    quality: {
      cycles_used: built.cyclesUsed,
      cycles_rejected: built.cyclesRejected,
      alignment_quality: built.alignmentQuality,
    },
  };
}
...
export function analyseRecording(buffer, durationMs) {
  ...
  if (fsHz < 20) {
    out.unavailableReason =
      `sample rate ${fsHz.toFixed(1)}Hz is too low for second-derivative analysis (need 20Hz+)`;
    return out;
  }
  if (beats.length < 5) {
    out.unavailableReason = `only ${beats.length} beats detected — need at least 5`;
    return out;
  }

  const r = analyseSdptg(buffer, beats, sampleIntervalMs);
  if (!r.ok) {
    out.unavailableReason = r.reason;
    return out;
  }

  const s = r.sdptg;
  const orderOk = s.a_frac < s.b_frac && s.b_frac < s.c_frac
               && s.c_frac < s.d_frac && s.d_frac < s.e_frac;
  const signsOk = s.b_a < 0 && s.d_a < 0;
  if (!orderOk || !signsOk) {
    out.unavailableReason =
      'SDPTG waves not in the expected order or sign for this recording, so the ratios are not trustworthy';
    return out;
  }

  // Three measured, dimensionless, directly comparable ratios. All normalised
  // against the same a-wave, so they combine with no scaling constant.
  const sharpRaw = Math.abs(s.b_a);
  const compRaw = Math.max(0, s.c_a);
  const rebRaw = Math.max(0, s.e_a);
  const sum = sharpRaw + compRaw + rebRaw;
  if (!(sum > 0)) {
    out.unavailableReason = 'all three SDPTG ratios measured as zero';
    return out;
  }

  const vata = Math.round((sharpRaw / sum) * 1000) / 10;
  const pitta = Math.round((compRaw / sum) * 1000) / 10;
  const kapha = Math.round((100 - vata - pitta) * 10) / 10;

  // Confidence describes the MEASUREMENT: how well beats phase-locked, how
  // many cycles were averaged, and the sample rate.
  let confidence = 20;
  confidence += 45 * r.quality.alignment_quality;
  confidence += Math.min(20, r.quality.cycles_used / 10);
  if (fsHz >= 50) confidence += 15;
  else if (fsHz >= 30) confidence += 8;
...
export { detectBeats, computeHrvFromBeats, analyseSdptg, mean, std };
